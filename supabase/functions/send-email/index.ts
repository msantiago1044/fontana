// send-email/index.ts
// Envía correos con Resend en 4 situaciones:
...
// Secrets: RESEND_API_KEY, RESEND_FROM (ej. "Fontana <hola@tudominio.com>")
import { createClient } from "https://esm.sh/@supabase/supabase-js@2";

const RESEND_KEY  = Deno.env.get("RESEND_API_KEY")!;
const FROM        = Deno.env.get("RESEND_FROM") ?? "Fontana <fontanadigital.ai@gmail.com>";
...
<body><div class="card">
  ...
  ${body}
  <div class="footer">
    Fe y trabajo · <a href="https://fontana-vert.vercel.app">fontana</a> ·
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (type === "welcome") {
      ...
      await sendResend(
        ...
        emailBase(`
          ...
          <p>Cuando estés listo, pide tu deseo desde <a href="https://fontana-vert.vercel.app">nuestra página</a>. Con una contribución desde $1 USD, el sistema de IA empieza a trabajar en él de inmediato.</p>
    ...
    // ── 2. Recordatorio 24h (disparado por pg_cron) ────────────────────────
    if (type === "reminder_24h") {
      // Buscar usuarios con deseo pending_payment de hace exactamente ~24h
      ...
      for (const w of pending ?? []) {
        await sendResend(
          ...
          emailBase(`
            ...
            <p>La fuente todavía está aquí. Si estás listo para dar el paso, solo necesitas completar la contribución desde $1 USD.</p>
      ...
      return new Response(JSON.stringify({ ok: true, sent: pending?.length ?? 0 }), {
        headers: { ...cors, "Content-Type": "application/json" },
      });
    }
    ...
    if (type === "payment_confirmed") {
      const { to, name, wishText, category } = body;
      const firstName = (name || "").split(" ")[0] || "viajero";
      ...
      await sendResend(
        ...
        emailBase(`
          ...
          <p>En las próximas 24 horas recibirás tu primer correo de acompañamiento: un paso concreto que puedes dar esta misma semana.</p>
          <p>Durante los próximos 30 días el sistema te irá enviando avances cada vez que detecte algo relevante para ti.</p>
          <p><em>Fe y trabajo.</em></p>
        `)
      );
    ...
    // ── 4. Respuesta manual tuya desde Telegram ────────────────────────────
    if (type === "manual_reply") {
      ...
      await sendResend(to, subject, emailBase(htmlBody));
